...
// IDs verified against TMDB and the title's official Netflix offer. Preserve
// them when the daily JustWatch shelf refreshes without OMDb enrichment.
const VERIFIED_JUSTWATCH_PLAYBACK = {
    'movie:vishwanath-and-sons': {
        tmdb_id: '1408162',
        embed_provider: 'nxsha',
        netflixId: '82034837',
        netflixUrl: 'https://www.netflix.com/in/title/82034837',
    },
};
...
/** Generate native poster assets and a URL-only manifest for the web bundle. */
function writePosterIndex() {
    ...
    // Metro otherwise puts every native require() (two per poster) into the
    // website's startup bundle, even though the web image path uses URLs.
    // A platform-specific module keeps the web map small and free of assets.
    const webOutput = [
        '/** Web-only poster URLs. Generated by refresh-catalog.mjs. */',
        'export const LOCAL_POSTER_URIS: Record<string, string> = {',
        uriEntries,
        '};',
        '',
        'export function getLocalPoster(key?: string | number): { uri: string } | undefined {',
        '    if (!key) return undefined;',
        '    const strKey = String(key).trim();',
        "    const cleanKey = strKey.replace(/^local:/, '');",
        '    const uri =',
    ...
    writeFileSync(join(POSTERS_DIR, 'index.web.ts'), `${webOutput}\n`);
}
...
async function scrapeJustWatchCatalog() {
    ...
    for (const { kind, path } of kinds) {
        ...
        const items = parsed.map((item, index) => ({
            ...
            ...VERIFIED_JUSTWATCH_PLAYBACK[`${kind}:${item.slug}`],
        }));
